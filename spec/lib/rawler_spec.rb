...
describe Rawler::Base do
  ...
  describe "validate_links" do
    ...
    it "should handle relative redirections" do
      register('http://example.com', '<a href="/foo">foo</a>')
      register('http://example.com/foo', '', 301, :location => 'bar')
  ...
  describe "get_status_code" do
    ...
    it "should rescue from general errors" do
      ...
      Rawler::Request.should_receive(:get).and_raise('foo')

      output.should_receive(:error).with("Unknown error foo (RuntimeError) - #{url} - Called from: #{from}")
